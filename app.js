...
function rowMapActions(r){const ticket=r['Suzuki QR']?`<a href="${esc(r['Suzuki QR'])}" target="_blank" rel="noopener">🎫 入場 QR</a>`:'';const guide=r['Guide']?`<a href="${esc(r['Guide'])}" target="_blank" rel="noopener">📖 ${esc(r['Guide Label']||'行前讀本')}</a>`:'';const guidePdf=r['Guide PDF']?`<a href="${esc(r['Guide PDF'])}" target="_blank" rel="noopener">📕 完整 PDF</a>`:'';return `${mapBtn(r['Google Maps'])}${r['Google Maps']?mapCode(r['Map Code'],r['Map Code Status']):mapCode(r['Map Code'])}${ticket}${guide}${guidePdf}`;}
